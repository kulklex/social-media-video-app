      ...
      <div className="bg-white rounded-lg xl:h-[80vh] flex gap-6 flex-wrap justify-center items-center p-14 pt-6">
        <div>
          ...
          <div className='border-dashed border-4 border-gray-200 rounded-xl flex flex-col justify-center items-center outline-none mt-10 w-[260px] h-[460px] p-10 cursor-pointer hover:border-red-300 hover:bg-gray-100'>
            ...
            : <div>
                {videoAsset 
                ? (<div className='rounded-3xl w-[270px] p-4 flex flex-col gap-6 justify-center items-center'>
                ...
                : (<label className='cursor-pointer'>
                  <div className='flex flex-col items-center justify-center h-full'>
                    ...
                    <p className='text-gray-400 text-center mt-10 text-sm leading-10'>
                      MP4 or WebM or ogg <br/> 720x1280 or higher <br/> Up to 10 minutes <br/> Less than 2GB 
                    </p>
        ...
        <div className='flex flex-col gap-3 pb-10'>
          ...
          <input name="caption" type="text" value={caption} onChange={(e) => setCaption(e.target.value)} className='rounded outline-none text-md border-2 border-gray-200 p-2'/>
          ...
          <select onChange={(e) => setCategory(e.target.value)} className='outline-none border-2 border-gray-200 text-md capitalize lg:p-4 p-2 rounded cursor-pointer'>
            {topics.map(({name}) => (
              <option key={name} value={name} className='outline-none capitalize bg-white text-gray-700 text-md hover:bg-slate-300'>
                {name}
              </option>
            ))}
          </select>
          ...
          <div className='flex gap-6 mt-10'>
            ...
            <button onClick={handlePost} disabled={videoAsset?.url ? false : true} type="button" className='bg-[#F51997] text-white border-gray-300 border-2 text-md font-medium p-2 rounded w-28 lg:w-44 outline-none'>
              Post
            </button>
          </div>
        </div>
